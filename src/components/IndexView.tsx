...
export const IndexView: React.FC<IndexViewProps> = ({
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      {/* Hero Section */}
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-violet-900 via-indigo-900 to-purple-950 text-white p-6 sm:p-10 shadow-xl border border-violet-700/30 mb-10">
        ...
        <div className="relative z-10 max-w-3xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 backdrop-blur-md border border-white/15 text-xs font-semibold text-purple-200 mb-4">
            <GraduationCap className="w-4 h-4 text-amber-300" />
            <span>NIMCET · MCA Entrance · JEE · Mathematics Formula Universe</span>
          </div>

          <h1 className="font-heading font-extrabold text-3xl sm:text-5xl tracking-tight leading-tight mb-4">
          ...
          <p className="text-purple-100/90 text-sm sm:text-base leading-relaxed mb-6 max-w-2xl">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2 border-t border-white/15">
            <div className="p-2.5 rounded-xl bg-white/5 backdrop-blur-sm border border-white/10">
              <span className="text-xs text-purple-300 block font-medium">Total Topics</span>
              <span className="text-xl sm:text-2xl font-bold font-mono text-white">{topics.length}</span>
            </div>
            <div className="p-2.5 rounded-xl bg-white/5 backdrop-blur-sm border border-white/10">
              <span className="text-xs text-purple-300 block font-medium">Formulas</span>
              <span className="text-xl sm:text-2xl font-bold font-mono text-cyan-300">~{totalFormulasCount}+</span>
            </div>
            <div className="p-2.5 rounded-xl bg-white/5 backdrop-blur-sm border border-white/10">
              <span className="text-xs text-purple-300 block font-medium">Starred</span>
              <button 
                onClick={onOpenFavorites}
                className="text-xl sm:text-2xl font-bold font-mono text-amber-300 hover:underline flex items-center gap-1"
              >
                <span>{starredFormulaIds.size}</span>
                <span className="text-xs font-normal text-amber-200/80">view →</span>
              </button>
            </div>
            <div className="p-2.5 rounded-xl bg-white/5 backdrop-blur-sm border border-white/10">
              <span className="text-xs text-purple-300 block font-medium">Mastery</span>
              <span className="text-xl sm:text-2xl font-bold font-mono text-emerald-300">{revisionPercent}%</span>
            </div>
          </div>
        </div>
        ...
        <div className="relative mt-8 max-w-2xl">
          <div className="relative flex items-center">
            <Search className="absolute left-4 w-5 h-5 text-purple-300 pointer-events-none" />
            <input
              type="text"
              value={globalSearch}
              onChange={(e) => setGlobalSearch(e.target.value)}
              placeholder="Search across all 18 topics (e.g., 'Bayes', 'Pythagorean', 'ILATE', 'AM-GM', 'Latus Rectum')..."
              className="w-full pl-12 pr-10 py-3.5 rounded-2xl bg-white/15 border border-white/20 text-white placeholder-purple-200/70 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 focus:bg-white/20 transition-all shadow-inner"
            />
            {globalSearch && (
              <button
          ...
          {globalSearch.trim() && (
            <div className="absolute left-0 right-0 top-full mt-2 bg-[var(--card)] text-[var(--ink)] rounded-2xl shadow-2xl border border-[var(--border)] z-30 max-h-96 overflow-y-auto p-2">
              ...
              {searchResults.length === 0 ? (
                ...
                searchResults.map((res) => (
                  <button
                    ...
                    className="w-full text-left p-3 hover:bg-[var(--card-hover)] rounded-xl transition-colors flex items-start justify-between gap-3 group border-b border-[var(--border)]/40 last:border-0"
                  >
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-sm">{res.topicIcon}</span>
                        ...
                        {res.mustKnow && (
                          <span className="bg-rose-500/15 text-rose-600 dark:text-rose-400 text-[10px] font-bold px-1.5 py-0.5 rounded">
                            Must Know
                          </span>
                        )}
                      </div>
                      <div className="font-semibold text-sm text-[var(--ink)] group-hover:text-purple-600 transition-colors">
                        {res.formulaTitle}
                      </div>
                      <div className="mt-1 max-w-lg overflow-x-auto text-[var(--ink-muted)]">
                        <MathRenderer
                          latex={res.latex}
        ...
        <div className="mt-6 flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-white/10">
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl text-xs font-bold bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-purple-950 shadow-md transition-all active:scale-95 flex items-center gap-1.5"
          >
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-heading text-lg font-bold text-[var(--ink)] flex items-center gap-2">
            <span>Explore Mathematical Topics</span>
            <span className="text-xs font-normal text-[var(--ink-muted)]">({filteredTopics.length} available)</span>
          </h2>
          {revisedTopicIds.size > 0 && (
            <span className="text-xs text-emerald-600 dark:text-emerald-400 font-semibold flex items-center gap-1">
              <CheckCircle2 className="w-3.5 h-3.5" />
              {revisedTopicIds.size} completed
            </span>
          )}
        </div>
        ...
        {filteredTopics.map((topic) => {
          ...
          return (
            <div
              ...
              className={`group relative rounded-2xl bg-[var(--card)] border transition-all duration-200 cursor-pointer overflow-hidden flex flex-col justify-between ${
                isRevised
              ...
              <div className="p-5 flex-1 flex flex-col justify-between">
                <div>
                  ...
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div className="flex items-center gap-3">
                      <div className="w-11 h-11 rounded-xl bg-purple-500/10 dark:bg-purple-500/20 flex items-center justify-center text-2xl shrink-0 group-hover:scale-110 transition-transform">
                        {topic.icon}
                      </div>
                      <div>
                        <h3 className="font-heading font-bold text-base text-[var(--ink)] group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">
                          {topic.name}
                        </h3>
                  ...
                  {/* Description */}
                  <p className="text-xs text-[var(--ink-muted)] line-clamp-2 leading-relaxed mb-4">
                    {topic.description}
                  </p>
                </div>
                ...
                <div>
                  <div className="flex flex-wrap items-center gap-2 mb-4 text-[11px]">
                    <span className="bg-purple-100 dark:bg-purple-950/50 text-purple-700 dark:text-purple-300 font-semibold px-2 py-0.5 rounded-md">
                      {formulaCount} Formulas
                    </span>
                    {mustKnowCount > 0 && (
                      <span className="bg-rose-100 dark:bg-rose-950/50 text-rose-700 dark:text-rose-300 font-semibold px-2 py-0.5 rounded-md flex items-center gap-1">
                        <Flame className="w-3 h-3 text-rose-500" />
                        {mustKnowCount} Must-Know
                      </span>
                    )}
                    <span className="text-[10px] text-[var(--ink-muted)] ml-auto font-medium">
                      {topic.examWeightage}
                    </span>
                  </div>
